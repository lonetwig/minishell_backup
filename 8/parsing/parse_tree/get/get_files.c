...
/*   Updated: 2024/02/29 17:53:56 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_str_list	*append_file(char *str, t_str_list *all_files, char **err)
{
	t_str_list	*file;

	file = ft_malloc(sizeof(t_str_list), 0, err);
	if (!file)
		return (NULL);
	file->str = ft_strdup(str, err);
	file->next = NULL;
	if (!all_files)
		all_files = file;
	else
...
void	get_files(t_str_list **all_files, char *path, char **err)
{
	...
	while (entry)
	{
		tmp = append_file(entry->d_name, tmp, err);
		if (!head)
			head = tmp;
		entry = readdir(dir);
	}
