...
/*   Updated: 2024/02/29 17:45:57 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	add_str_to_arr(t_split *all, int *s_i, int arr_i, char **err)
{
	all->arr[arr_i] = ft_malloc(str_size(all, *s_i) + 1, 0, err);
...
int	split_by_space2(t_split *all, char **err)
{
	...
	if (str[s_i] && str[s_i] != ' ')
		add_str_to_arr(all, &s_i, arr_i++, err);
	s_i++;
	while (all->str[s_i])
	{
