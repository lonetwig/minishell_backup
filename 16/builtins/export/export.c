...
/*   Updated: 2024/04/17 15:24:43 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**parse_export(char **arg, int k, t_all *all)
{
	...
	i = 0;
	key_val = ft_malloc_permanent(sizeof(char *) * 2, 0, all);
	if (!key_val)
		return (NULL);
	...
	{
		key_val[0] = ft_strdup(arg[k], all, ft_malloc_permanent);
		if (!key_val[0])
			return (NULL);
		key_val[1] = NULL;
	}
...
static void	parse_key(char **key_val, t_all *all)
{
	...
	{
		all->tmp = ft_strjoin(key_val[0], "= is not a valid undentifier", all);
		exit_stat(1, 1);
		return ;
	}
...
void	export(t_all *all, t_env *env, char **arg)
{
	...
	if (all->tmp)
		all->error = ft_strdup(all->tmp, all, ft_malloc);
	exit_stat(0, 1);
}
