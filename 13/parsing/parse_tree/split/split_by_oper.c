...
/*   Updated: 2024/03/04 15:04:26 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	append_str(char *str, char **arr, int size, char **error)
{
	char	*new_str;
	int	i;
	...
	// 	return ;
	// }
	new_str = ft_malloc(size + 1, 0, error);
	if (!new_str)
		return ;
	while (i < size)
	{
...
char	**split_by_oper(char *str, e_oper oper_type, char c[2], char **err)
{
	...
	l_r_sizes[1] = 0;
	l_r_sizes[2] = 0;
	arr = ft_malloc(3 * sizeof (char *), 0, err);
	if (!arr)
		return (NULL);
	get_lens(str, oper_type, c, l_r_sizes);
	append_str(str, arr, l_r_sizes[0], err);
	append_str(str + l_r_sizes[0] + l_r_sizes[1], arr + 1, l_r_sizes[2], err);
	arr[2] = NULL;
	return (arr);
}
