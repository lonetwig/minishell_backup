...
/*   Updated: 2024/05/12 17:28:06 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../minibash.h"

// used to check what type is the oper, and append it to oper array
// redirectionals = 1 | opers = 2
static int	oper_type(char *str, int *i, int oper[2])
{
	int	oper_i;
...
// used to initialise an int array with zeros
...
// used to print and set the stat
...
//used to check if all opers (including redirection) are correct
void	check_opers(char *str, t_all *all)
{
	...
	while (str[i])
	{
		...
		if (is_white_space(str[i]) && ++i)
			continue ;
		if (!oper_type(str + i, &i, oper) || cond_q(c_q))
			fill_arr_with_zero(oper);
