...
/*   Updated: 2024/03/02 17:00:34 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_fds(char *s, int *input, int *output, char **err)
{
	...
	if (!*err)
	{
		*input = get_input_output(arr, 0, NULL, err);
		*output = get_input_output(arr, 1, NULL, err);
	}
	// free_arr(arr);
}
...
static void	check_str(char *s, int *input, int *output, char **error)
{
	int				i;
	int				p_i;
	t_check_quote	c_q;
