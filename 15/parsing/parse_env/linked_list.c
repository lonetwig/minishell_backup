...
/*   Updated: 2024/04/17 15:22:57 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_env	*create_node(char *key, char *value, t_all *all)
{
	...
	env = ft_malloc_permanent(sizeof(t_env), 0, all);
	if (!env)
	{
		ft_write("Malloc error", 2, 1);
		exit(2);
	}
	env->key = ft_strdup(key, all, ft_malloc_permanent);
	if (!env->key)
	{
		ft_write("Malloc error", 2, 1);
		exit(3);
	...
	if (value)
	{
		env->value = ft_strdup(value, all, ft_malloc_permanent);
		if (!env->value)
		{
			ft_write("Malloc error", 2, 1);
			exit(4);
