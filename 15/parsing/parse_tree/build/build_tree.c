...
/*   Updated: 2024/04/17 17:22:24 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_tree	*add_branch(char **l_r, t_e_oper oper, int *doc_i, t_all *all)
{
	...
	*tree = (t_tree){0};
	...
	{
		tree->cmd_str = ft_strdup(l_r[0], all, ft_malloc);
		tree->doc_i = *doc_i;
...
static void	handle_parentheses(void **data, t_all *all, t_e_oper opr, int *doc_i)
{
	...
	*tree = add_branch(NULL, opr, doc_i, all);
	...
	if (all->error)
		return ;
	str2 = split_by_parantheses(str, &(*tree)->input, &(*tree)->output, all);
	if (all->error || !str2)
		return ;
	...
	if (!str2)
		return ;
	build_tree(str2, (t_tree **[2]){&(*tree)->left, root}, all, doc_i);
}
