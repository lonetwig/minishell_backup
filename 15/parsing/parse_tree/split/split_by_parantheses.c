...
/*   Updated: 2024/04/17 16:30:37 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_fds(char *s, int *input, int *output, t_all *all)
{
	...
	while (arr && arr[i])
	{
		...
		cond_redirect(arr[i + 1])))
			return (all->error = "minibash:d syntax error", exit_stat(1, 1), 0);
		i += 2;
	}
...
static int	check_str(char *s, int *input, int *output, t_all *all)
{
	...
	while (s[i] == ' ')
		i++;
	if (s[i] != '(')
		return (all->error = "minibash: syntax error", exit_stat(1, 1), 1);
	while (s[i])
	{
	...
	while (s[i] == ' ')
		i++;
	if (s[i])
		check_end_and_get_fds(s + i, input, output, all);
	return (0);
}
...
static int	get_para_size(char *str, t_all *all)
{
	...
	if (p_i)
		(all->error = "minibash: syntax error", exit_stat(1, 1));
	return (size);
}
