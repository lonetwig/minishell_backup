...
/*   Updated: 2024/04/17 15:24:10 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_append(t_env *tmp, char *key_val, int append, t_all *all)
{
	if (append)
		...
		tmp->value = ft_strdup(key_val, all, ft_malloc_permanent);
}
...
void	change_val(t_env **env, char **key_val, int append, t_all *all)
{
	...
	while (tmp)
	{
		if (!ft_strncmp(tmp->key, key_val[0], ft_strlen(tmp->key) && key_val[1]))
		{
			all->tmp_val = ft_strdup(tmp->value, all, ft_malloc_permanent);
			// free(tmp->value);
			if (key_val[1])
			{
